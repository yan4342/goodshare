...
class HybridRecommender:
    ...
    def train(self):
        ...
        logger.info("\n开始训练物品协同过滤模型 (ItemCF)...")
        # ItemCF: item-item similarity from user interaction patterns
        item_sim_matrix = cosine_similarity(self.user_item_matrix.T)
        self.item_cf_similarity_df = pd.DataFrame(
            item_sim_matrix,
    ...
    def _train_content_similarity(self):
        ...
        if content_df.empty:
            self.item_similarity_df = None
            logger.warning("未找到帖子内容数据。内容过滤将失效。")
            return
        
        # 构建项目内容矩阵，用 TF-IDF 提取帖子文本特征
    ...
    # 基于内容过滤推荐
    def _recommend_content_based(self, user_id, top_k=20, max_seed_items=20, max_sim_items=50):
        if self.user_item_matrix is None or self.item_similarity_df is None:
            logger.debug(f"内容过滤被跳过：矩阵未初始化 (user_item_matrix is None: {self.user_item_matrix is None}, item_similarity_df is None: {self.item_similarity_df is None})")
            return []
        if user_id not in self.user_item_matrix.index:
            logger.debug(f"内容过滤被跳过：用户 {user_id} 没有交互记录")
            return []
        
        target_vector = self.user_item_matrix.loc[user_id]
        interacted = target_vector[target_vector > 0].sort_values(ascending=False)
        if interacted.empty:
            logger.debug(f"内容过滤被跳过：用户 {user_id} 交互分数均小于等于 0")
            return []
    ...
    # 基于物品协同过滤推荐 (ItemCF)
    def _recommend_item_cf(self, user_id, top_k=20, max_seed_items=20, max_sim_items=50):
        if self.user_item_matrix is None or self.item_cf_similarity_df is None:
            logger.debug(f"ItemCF被跳过：矩阵未初始化 (user_item_matrix is None: {self.user_item_matrix is None}, item_cf_similarity_df is None: {self.item_cf_similarity_df is None})")
            return []
        if user_id not in self.user_item_matrix.index:
            logger.debug(f"ItemCF被跳过：用户 {user_id} 没有交互记录")
            return []

        target_vector = self.user_item_matrix.loc[user_id]
        interacted = target_vector[target_vector > 0].sort_values(ascending=False)
        if interacted.empty:
            logger.debug(f"ItemCF被跳过：用户 {user_id} 交互分数均小于等于 0")
            return []
        ...
        for seed_id in seed_items:
            if seed_id not in self.item_cf_similarity_df.index:
                continue
            sim_series = self.item_cf_similarity_df.loc[seed_id].sort_values(ascending=False).iloc[1:max_sim_items+1]
            seed_weight = interacted.loc[seed_id]
            for item_id, sim_score in sim_series.items():
                if item_id in target_user_items:
                    continue
    ...
    def recommend(self, user_id, top_k=20, n_similar_users=10, user_cf_weight=0.4, content_cf_weight=0.2, item_cf_weight=0.4):
        ...
        item_cf_recs = self._recommend_item_cf(user_id, top_k=top_k)

        logger.info(f"召回结果 - UserCF: {len(user_cf_recs)} 个, ContentCF: {len(content_cf_recs)} 个, ItemCF: {len(item_cf_recs)} 个")
        ...
        for rec in item_cf_recs:
            ...
            combined[post_id] = combined.get(post_id, 0) + score
            details.setdefault(post_id, {})["item_cf"] = score
        ...
        for i, res in enumerate(top_results):
            ...
            # 判断主要来源
            i_score = detail.get('item_cf', 0.0)

            sources = []
            if u_score > 0:
                sources.append("UserCF")
            if c_score > 0:
                sources.append("ContentCF")
            if i_score > 0:
                sources.append("ItemCF")
            source = " + ".join(sources) if sources else "未知"

            # 获取帖子标签
            tags = self.post_tags_map.get(post_id, "") if hasattr(self, 'post_tags_map') else ""
            tags_display = f" | 标签: [{tags}]" if tags else ""

            logger.info(f"  {i+1}. 帖子ID: {post_id}{tags_display} | 总分: {score:.4f} (UserCF: {u_score:.4f}, ContentCF: {c_score:.4f}, ItemCF: {i_score:.4f}) | 来源: {source}")
